const fs = require('fs');
const path = 'C:/Users/cana2/OneDrive/Desktop/thucnganv2-main/tonkho-thucte.js';
...
function renderTonKhoThucTeTab(taxCode) {
  ...
  html += '<option value="auto"' + (_ttFilterStatus === 'auto' ? ' selected' : '') + '>Tu dong</option>';
  html += '<option value="semi_strong"' + (_ttFilterStatus === 'semi_strong' ? ' selected' : '') + '>Ban tu dong >=80%</option>';
  html += '<option value="semi"' + (_ttFilterStatus === 'semi' ? ' selected' : '') + '>Ban tu dong 50-79%</option>';
  html += '<option value="manual"' + (_ttFilterStatus === 'manual' ? ' selected' : '') + '>Thu cong</option>';
  html += '</select>';
  ...
  html += (_ttSortDir === 'asc' ? 'U+2B06' : 'U+2B07');
  html += '</button>';
  ...
  html += '<th class="tt-col-name">Ten san pham</th>';
  html += '<th class="tt-col-qty">Ton kho</th>';
  html += '<th class="tt-col-qty">Xuat kho</th>';
  ...
  if (filtered.length === 0) {
    html += '<tr><td colspan="8" style="text-align:center;padding:30px;color:#999;">Khong co du lieu phu hop</td></tr>';
  } else {
    for (var i = 0; i < filtered.length; i++) {
      ...
      var statusBadge = '';
      if (r.status === 'auto') { statusBadge = '<span class="tt-status-badge" style="background:#2e7d32;">Tu dong</span>'; }
      else if (r.status === 'semi_strong') { statusBadge = '<span class="tt-status-badge" style="background:#1565c0;">Ban tu dong ' + r.matchPercent + '%</span>'; }
      else if (r.status === 'semi') { statusBadge = '<span class="tt-status-badge" style="background:#e65100;">Ban tu dong ' + r.matchPercent + '%</span>'; }
      else { statusBadge = '<span class="tt-status-badge" style="background:#9e9e9e;">Thu cong</span>'; }
      ...
      var actionBtns = '';
      if (r.status === 'auto') { actionBtns = '<span style="color:#2e7d32;font-size:0.85em;">Da ghep</span>'; }
      else if (r.status === 'semi_strong' || r.status === 'semi') {
        actionBtns = '<button class="tt-btn tt-btn-compare" onclick="showTTComparePopup(\\'' + taxCode + '\\',' + r.xuatIdx + ',' + r.tonkhoIdx + ',' + r.matchPercent + ')">So sanh</button>';
      } else {
        actionBtns = '<button class="tt-btn tt-btn-manual" onclick="showTTManualPopup(\\'' + taxCode + '\\',' + r.xuatIdx + ')">Ghep thu cong</button>';
      }
      ...
      if ((r.status === 'semi' || r.status === 'semi_strong' || r.status === 'manual') && r.suggestions && r.suggestions.length > 0) {
        ...
        suggestionsHtml += '<span class="tt-suggestions-label">Goi y:</span>';
...
function batchMatchSelected(taxCode) {
  if (_ttSelectedItems.size === 0) {
    window.showToast('Chua chon muc nao', 2000, 'warning');
    return;
  }
  var results = calculateTonKhoThucTe(taxCode);
  var matched = 0;
  ...
  _ttSelectedItems = new Set();
  window.showToast('Da ghep ' + matched + ' muc thanh cong!', 3000, 'success');
...
function onTTDrop(event, taxCode, tonkhoIdx) {
  ...
  saveManualMatchCache(taxCode);
  window.showToast('Da ghep keo tha thanh cong!', 2000, 'success');
  renderTonKhoThucTeTab(taxCode);
}
...
function showTTComparePopup(taxCode, xuatIdx, tonkhoIdx, matchPercent) {
  ...
  html += '<div style="font-weight:600;color:#e65100;margin-bottom:8px;">Xuat kho</div>';
  ...
  html += '<div style="font-weight:600;color:#1976d2;margin-bottom:8px;">Ton kho</div>';
  ...
  html += '<span style="display:inline-block;padding:4px 16px;border-radius:20px;background:' + getMatchColor(matchPercent) + ';color:#fff;font-weight:600;">Ti le tuong dong: ' + matchPercent + '%</span>';
  html += '</div>';

  if (suggestions.length > 0) {
    html += '<div style="margin-bottom:16px;">';
    html += '<div style="font-weight:600;margin-bottom:8px;color:#666;">Cac goi y khac:</div>';
  ...
  html += '<button onclick="confirmTTMatch(\\'' + taxCode + '\\',' + xuatIdx + ',' + tonkhoIdx + ')" style="padding:10px 24px;background:#2e7d32;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600;">Dong y ghep</button>';
  html += '<button onclick="rejectTTMatch(\\'' + taxCode + '\\',' + xuatIdx + ')" style="padding:10px 24px;background:#d32f2f;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600;">Tu choi</button>';
  html += '<button onclick="window.closePopup()" style="padding:10px 24px;background:#e0e0e0;color:#333;border:none;border-radius:8px;cursor:pointer;">Huy</button>';
  html += '</div></div>';

  window.showPopup(html, 'So sanh san pham');
}
...
function confirmTTMatch(taxCode, xuatIdx, tonkhoIdx) {
  ...
  saveManualMatchCache(taxCode);
  window.closePopup();
  window.showToast('Da ghep thanh cong!', 2000, 'success');
  renderTonKhoThucTeTab(taxCode);
}
...
function rejectTTMatch(taxCode, xuatIdx) {
  ...
  saveManualMatchCache(taxCode);
  window.closePopup();
  window.showToast('Da tu choi ghep', 2000, 'info');
  renderTonKhoThucTeTab(taxCode);
}
...
function showTTManualPopup(taxCode, xuatIdx) {
  ...
  var html = '<div style="padding:20px;">';
  html += '<h3 style="margin-bottom:16px;">Ghep thu cong</h3>';
  html += '<div style="padding:12px;background:#fff3e0;border-radius:8px;margin-bottom:16px;">';
  html += '<div><strong>Xuat kho:</strong> ' + escapeHtml(xuatItem.productName || '') + '</div>';
  html += '<div style="color:#666;">DVT: ' + escapeHtml(xuatItem.unit || '') + ' | SL: ' + formatQuantity(xuatItem.quantity) + '</div>';
  html += '</div>';
  html += '<input type="text" id="tt-manual-search" placeholder="Tim san pham ton kho..." oninput="filterTTManualItems()" style="width:100%;padding:8px 12px;border:1px solid #ccc;border-radius:6px;margin-bottom:12px;">';
  ...
  html += '<button id="tt-manual-confirm-btn" onclick="confirmTTManual(\\'' + taxCode + '\\',' + xuatIdx + ')" disabled style="padding:10px 24px;background:#7b1fa2;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600;opacity:0.5;">Ghep</button>';
  html += '<button onclick="window.closePopup()" style="padding:10px 24px;background:#e0e0e0;color:#333;border:none;border-radius:8px;cursor:pointer;">Huy</button>';
  html += '</div></div>';

  window.showPopup(html, 'Ghep thu cong');
...
function confirmTTManual(taxCode, xuatIdx) {
  ...
  if (tonkhoIdx === undefined || tonkhoIdx < 0) {
    window.showToast('Vui long chon san pham ton kho', 2000, 'warning');
    return;
  }
  var hkd = hkdData[taxCode];
  if (!hkd) return;
  ...
  saveManualMatchCache(taxCode);
  window.closePopup();
  window.showToast('Da ghep thu cong thanh cong!', 2000, 'success');
  renderTonKhoThucTeTab(taxCode);
}
...
function showQuickMatchPopup(taxCode) {
  ...
  if (manualItems.length === 0) {
    window.showToast('Khong co item nao can ghep', 2000, 'info');
    return;
  }

  var html = '<div style="padding:20px;">';
  html += '<h3 style="margin-bottom:16px;">Ghep nhanh (' + manualItems.length + ' item)</h3>';
  html += '<p style="color:#666;margin-bottom:16px;">Chon san pham ton kho phu hop cho tung item xuat kho:</p>';
  ...
  for (var i = 0; i < manualItems.length; i++) {
    ...
    if (r.suggestions && r.suggestions.length > 0) {
      ...
      for (var si = 0; si < r.suggestions.length; si++) {
        var sug = r.suggestions[si];
        var sugColor = getMatchColor(sug.percent);
        html += '<button onclick="quickMatchConfirm(\\'' + taxCode + '\\',' + r.xuatIdx + ',' + sug.idx + ')" style="padding:4px 10px;border:1px solid ' + sugColor + ';border-radius:6px;background:#fff;color:' + sugColor + ';cursor:pointer;font-size:0.85em;">';
        html += escapeHtml(sug.name) + ' (' + sug.percent + '%)';
        html += '</button>';
      }
    } else {
      html += '<div style="color:#999;font-size:0.85em;">Khong co goi y tu dong</div>';
    }
    html += '</div>'; // close suggestions div
    html += '</div>'; // close manual item div
  }
  html += '</div>'; // close popup body
  html += '</div>'; // close popup wrapper
  window.showPopup(html, 'Ghep nhanh');
}
...
function quickMatchConfirm(taxCode, xuatIdx, tonkhoIdx) {
  ...
  _manualMatchCache[taxCode][cacheKey] = { tonkhoIdx: tonkhoIdx, matchPercent: 100, timestamp: Date.now() };
  saveManualMatchCache(taxCode);
  window.showToast('Da ghep nhanh thanh cong!', 2000, 'success');
  renderTonKhoThucTeTab(taxCode);
}
`;

fs.appendFileSync(path, part2, 'utf8');
